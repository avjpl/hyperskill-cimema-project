...
public class CinemaTests extends StageTest<String> {
    ...
    CheckResult test1_checkExit() {
        ...
        String originalOutput = program.execute("8");
        output = originalOutput.toLowerCase();

        if (!output.contains("show the seats") ||
                !output.contains("buy a ticket") ||
                !output.contains("exit")) {
            return CheckResult.wrong("Expected your program to print the following menu items:\n" +
                    "1. Show the seats\n" +
                    "2. Buy a ticket\n" +
                    "0. Exit\n\n" +
                    "But your program's output was:\n" + originalOutput);
        }
    ...
    private void checkSeats(String output, int rows, int columns) {
        ...
        if (!output.toLowerCase().contains("cinema:")) {
            String expectedOutput = buildExpectedCinemaLayout(rows, columns);
            throw new WrongAnswer("Expected your program to print 'Cinema:' followed by the seating arrangement:\n" +
                    expectedOutput + "\n\n" +
                    "But your program's output was:\n" + output);
        }
    ...
    private String buildExpectedCinemaLayout(int rows, int columns) {
        StringBuilder expectedLayout = new StringBuilder("Cinema:\n  ");
        for (int i = 1; i <= columns; i++) {
            expectedLayout.append(i).append(" ");
        }
        expectedLayout.append("\n");
        for (int r = 1; r <= rows; r++) {
            expectedLayout.append(r);
            expectedLayout.append(" S".repeat(Math.max(0, columns)));
            if (r < rows) {
                expectedLayout.append("\n");
            }
        }
        return expectedLayout.toString();
    }
